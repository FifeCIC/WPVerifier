...
( function () {
	...
	document.addEventListener( 'DOMContentLoaded', function () {
		...
		form.addEventListener( 'submit', function ( event ) {
			...
			// Add checkbox values
			const checkWporg = document.getElementById( 'check_wporg' );
			const checkDomain = document.getElementById( 'check_domain' );
			if ( checkWporg && checkWporg.checked ) {
				formData.append( 'check_wporg', '1' );
			}
			if ( checkDomain && checkDomain.checked ) {
				formData.append( 'check_domain', '1' );
			}
			...
			fetch( pluginCheckNamer.ajaxUrl, {
				...
				.then( function ( payload ) {
					...
					// Display WordPress.org result if available
					const wporgResultDiv = document.getElementById( 'plugin-check-namer-wporg-result' );
					const wporgStatus = document.getElementById( 'plugin-check-namer-wporg-status' );
					if ( payload.data.wporg && wporgResultDiv && wporgStatus ) {
						const wporg = payload.data.wporg;
						let statusHtml = '<span class="dashicons ' + 
							( wporg.status === 'available' ? 'dashicons-yes' : 'dashicons-no' ) + 
							'" style="color:' + ( wporg.status === 'available' ? 'green' : 'red' ) + ';"></span> ';
						statusHtml += escapeHtml( wporg.message );
						if ( wporg.plugin_url ) {
							statusHtml += '<br><a href="' + escapeHtml( wporg.plugin_url ) + '" target="_blank">' + 
								escapeHtml( wporg.plugin_url ) + '</a>';
						}
						wporgStatus.innerHTML = statusHtml;
						wporgResultDiv.classList.remove( 'plugin-check-namer-hidden' );
					}

					// Display domain result if available
					const domainResultDiv = document.getElementById( 'plugin-check-namer-domain-result' );
					const domainStatus = document.getElementById( 'plugin-check-namer-domain-status' );
					if ( payload.data.domain && domainResultDiv && domainStatus ) {
						const domain = payload.data.domain;
						let statusHtml = '<span class="dashicons ' + 
							( domain.status === 'available' ? 'dashicons-yes' : 'dashicons-no' ) + 
							'" style="color:' + ( domain.status === 'available' ? 'green' : 'red' ) + ';"></span> ';
						statusHtml += escapeHtml( domain.message );
						if ( domain.note ) {
							statusHtml += '<br><em>' + escapeHtml( domain.note ) + '</em>';
						}
						domainStatus.innerHTML = statusHtml;
						domainResultDiv.classList.remove( 'plugin-check-namer-hidden' );
					}

					// Only show AI results if available
					if ( payload.data.verdict && payload.data.explanation ) {
					...
					} else if ( payload.data.ai_error ) {
						// Show AI error message if AI analysis failed but other checks succeeded
						if ( verdictContainer ) {
							setText( verdictEl, 'AI Analysis Unavailable' );
							setHtml( explainEl, escapeHtml( payload.data.ai_error ) );
							verdictContainer.style.borderLeftColor = '#dba617';
