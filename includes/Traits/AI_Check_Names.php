...
trait AI_Check_Names {
	...
	protected function run_name_analysis( $provider, $api_key, $model, $name, $author = '' ) {
		if ( ! class_exists( AiClient::class ) ) {
			return new WP_Error( 'ai_client_not_available', __( 'AI client SDK is not available.', 'wp-verifier' ) );
		}
	...
	protected function get_prompt_template( $filename = 'ai-check-similar-name.md' ) {
		if ( ! defined( 'WP_PLUGIN_CHECK_PLUGIN_DIR_PATH' ) ) {
			return new WP_Error( 'plugin_constant_not_defined', __( 'Plugin constant not defined.', 'wp-verifier' ) );
		}

		$path = WP_PLUGIN_CHECK_PLUGIN_DIR_PATH . 'prompts/' . $filename;
		if ( ! file_exists( $path ) ) {
			return new WP_Error( 'prompt_not_found', __( 'Prompt template not found.', 'wp-verifier' ) );
		}
		...
		if ( empty( $contents ) ) {
			return new WP_Error( 'prompt_empty', __( 'Prompt template is empty.', 'wp-verifier' ) );
		}
	...
	protected function parse_analysis( $analysis ) {
		...
		if ( empty( $analysis_text ) ) {
			return array(
				'verdict'     => '❓ ' . __( 'Empty Response', 'wp-verifier' ),
				'explanation' => __( 'The AI did not return any analysis. Please try again.', 'wp-verifier' ),
			);
		}
		...
		return array(
			'verdict'     => '❓ ' . __( 'Unable to Parse', 'wp-verifier' ),
			'explanation' => wp_kses_post( __( 'The AI response could not be parsed. Raw response:', 'wp-verifier' ) . '<br><br>' . esc_html( substr( $analysis_trim, 0, 500 ) ) ),
			'raw'         => $analysis_trim,
		);
	}
	...
	protected function parse_prereview_response( $decoded ) {
		...
		$explanation       = ! empty( $explanation_parts ) ? implode( '<br><br>', $explanation_parts ) : __( 'No detailed analysis available.', 'wp-verifier' );
	...
	protected function build_verdict( $decoded ) {
		...
		if ( $is_disallowed ) {
			return '❌ ' . __( 'Disallowed', 'wp-verifier' );
		}

		if ( ! empty( $issues ) ) {
			return '⚠️ ' . __( 'Issues Found', 'wp-verifier' ) . ': ' . implode( ', ', $issues );
		}
		...
		if ( $has_suggestions || $has_trademarks ) {
			return 'ℹ️ ' . __( 'Generally Allowable', 'wp-verifier' );
		}

		return '✅ ' . __( 'No Issues Detected', 'wp-verifier' );
	}
	...
	protected function collect_issues( $decoded ) {
		...
		if ( ! empty( $decoded['possible_naming_issues'] ) ) {
			$issues[] = __( 'Naming', 'wp-verifier' );
		}
		if ( ! empty( $decoded['possible_owner_issues'] ) ) {
			$issues[] = __( 'Owner/Trademark', 'wp-verifier' );
		}
		if ( ! empty( $decoded['possible_description_issues'] ) ) {
			$issues[] = __( 'Description', 'wp-verifier' );
		}
	...
	protected function add_disallowed_section( &$parts, $decoded ) {
		...
		if ( ! empty( $text ) ) {
			$parts[] = '<strong>' . __( '🚫 Disallowed:', 'wp-verifier' ) . '</strong> ' . $text;
		}
	}
	...
	protected function add_naming_section( &$parts, $decoded ) {
		if ( ! empty( $decoded['possible_naming_issues'] ) && ! empty( $decoded['naming_explanation'] ) ) {
			$parts[] = '<strong>' . __( '📝 Naming:', 'wp-verifier' ) . '</strong> ' . $decoded['naming_explanation'];
		}
	}
	...
	protected function add_owner_section( &$parts, $decoded ) {
		if ( ! empty( $decoded['possible_owner_issues'] ) && ! empty( $decoded['owner_explanation'] ) ) {
			$parts[] = '<strong>' . __( '©️ Owner/Trademark:', 'wp-verifier' ) . '</strong> ' . $decoded['owner_explanation'];
		}
	}
	...
	protected function add_description_section( &$parts, $decoded ) {
		if ( ! empty( $decoded['possible_description_issues'] ) && ! empty( $decoded['description_explanation'] ) ) {
			$parts[] = '<strong>' . __( '📄 Description:', 'wp-verifier' ) . '</strong> ' . $decoded['description_explanation'];
		}
	}
	...
	protected function add_trademarks_section( &$parts, $decoded ) {
		if ( ! empty( $decoded['trademarks_or_project_names_array'] ) && is_array( $decoded['trademarks_or_project_names_array'] ) ) {
			...
			$parts[]    = '<strong>' . __( '™️ Trademarks Detected:', 'wp-verifier' ) . '</strong> ' . $trademarks;
		}
	}
	...
	protected function add_suggestions_section( &$parts, $decoded ) {
		...
		if ( ! empty( $decoded['suggested_display_name'] ) ) {
			$suggestions[] = '<strong>' . __( 'Display Name:', 'wp-verifier' ) . '</strong> ' . esc_html( $decoded['suggested_display_name'] );
		}
		if ( ! empty( $decoded['suggested_slug'] ) ) {
			$suggestions[] = '<strong>' . __( 'Slug:', 'wp-verifier' ) . '</strong> ' . esc_html( $decoded['suggested_slug'] );
		}
		if ( ! empty( $decoded['short_description'] ) ) {
			$suggestions[] = '<strong>' . __( 'Description:', 'wp-verifier' ) . '</strong> ' . esc_html( $decoded['short_description'] );
		}
		if ( ! empty( $decoded['plugin_category'] ) ) {
			$suggestions[] = '<strong>' . __( 'Category:', 'wp-verifier' ) . '</strong> ' . esc_html( $decoded['plugin_category'] );
		}

		if ( ! empty( $suggestions ) ) {
			$parts[] = '<br><strong>' . __( '💡 Suggestions:', 'wp-verifier' ) . '</strong><br>' . implode( '<br>', $suggestions );
		}
	}
	...
	protected function add_language_section( &$parts, $decoded ) {
		if ( isset( $decoded['description_language_is_in_english'] ) && false === $decoded['description_language_is_in_english'] ) {
			if ( ! empty( $decoded['description_what_is_not_in_english'] ) ) {
				$parts[] = '<strong>' . __( '🌐 Language:', 'wp-verifier' ) . '</strong> ' . $decoded['description_what_is_not_in_english'];
			}
		}
	}
