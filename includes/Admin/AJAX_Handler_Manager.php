...
class AJAX_Handler_Manager {
	...
	public function check_domains() {
		...
		try {
			...
			if ( empty( $name ) ) {
				throw new \InvalidArgumentException( esc_html__( 'Name is required.', 'wpverifier' ) );
			}
	...
	public function save_name() {
		...
		try {
			...
			wp_send_json_success( array(
				'message' => __( 'Name saved successfully.', 'wpverifier' ),
			) );
	...
	public function start_monitoring() {
		...
		try {
			...
			if ( empty( $plugin ) ) {
				throw new \InvalidArgumentException( esc_html__( 'Plugin is required.', 'wpverifier' ) );
			}
			...
			wp_send_json_success( array(
				'message' => __( 'Monitoring started successfully.', 'wpverifier' ),
			) );
	...
	public function stop_monitoring() {
		...
		try {
			...
			wp_send_json_success( array(
				'message' => __( 'Monitoring stopped.', 'wpverifier' ),
			) );
	...
	public function mark_complete() {
		...
		try {
			...
			if ( empty( $plugin ) || empty( $file ) || empty( $code ) ) {
				throw new \InvalidArgumentException( esc_html__( 'Missing required parameters.', 'wpverifier' ) );
			}
			...
			wp_send_json_success( array(
				'message' => __( 'Issue marked as complete.', 'wpverifier' ),
			) );
	...
	public function add_ignore_rule() {
		...
		try {
			...
			if ( empty( $plugin_slug ) || empty( $file ) || empty( $code ) ) {
				throw new \InvalidArgumentException( esc_html__( 'Plugin, file, line, and code are required.', 'wpverifier' ) );
			}
			...
			if ( ! file_exists( $json_file ) ) {
				throw new \InvalidArgumentException( esc_html__( 'Results file not found.', 'wpverifier' ) );
			}
			
			$data = json_decode( file_get_contents( $json_file ), true );
			if ( ! $data || ! isset( $data['results'] ) ) {
				throw new \InvalidArgumentException( esc_html__( 'Invalid results file.', 'wpverifier' ) );
			}
			...
			if ( ! $updated ) {
				throw new \InvalidArgumentException( esc_html__( 'Issue not found in results.', 'wpverifier' ) );
			}
	...
	public function add_ignore_directory() {
		...
		try {
			...
			if ( empty( $plugin_slug ) || empty( $directory ) ) {
				throw new \InvalidArgumentException( esc_html__( 'Plugin and directory are required.', 'wpverifier' ) );
			}
			...
			wp_send_json_success( array(
				'message' => __( 'Directory ignore rule added successfully.', 'wpverifier' ),
			) );
	...
	public function get_scan_history() {
		...
		try {
			...
			if ( empty( $plugin_slug ) ) {
				throw new \InvalidArgumentException( esc_html__( 'Plugin slug is required.', 'wpverifier' ) );
			}
	...
	public function clear_scan_history() {
		...
		try {
			...
			wp_send_json_success( array(
				'message' => __( 'Scan history cleared successfully.', 'wpverifier' ),
			) );
	...
	public function mark_ignored() {
		...
		try {
			...
			if ( empty( $plugin ) || empty( $issue_id ) ) {
				throw new \InvalidArgumentException( esc_html__( 'Plugin and issue ID are required.', 'wpverifier' ) );
			}
			...
			if ( ! file_exists( $json_file ) ) {
				...
				wp_send_json_success( array(
					'message' => __( 'No verification results found for this plugin. Please run a verification scan first.', 'wpverifier' ),
					'action' => 'no_results'
				) );
				return;
			}
	...
	private function verify_request( $nonce ) {
		if ( ! wp_verify_nonce( $nonce, 'plugin-check-run-checks' ) ) {
			return new \WP_Error( 'invalid-nonce', esc_html__( 'Invalid nonce', 'wpverifier' ) );
		}

		if ( ! current_user_can( 'activate_plugins' ) ) {
			return new \WP_Error( 'invalid-permissions', esc_html__( 'Invalid user permissions, you are not allowed to perform this request.', 'wpverifier' ) );
		}
