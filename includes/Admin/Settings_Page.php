...
final class Settings_Page {
	...
	public function add_hooks() {
		// Don't add separate settings page - now integrated into main page
	...
	public function enqueue_admin_scripts( $hook_suffix ) {
		...
		wp_enqueue_style(
			'wp-verifier-tabs',
			plugins_url( 'assets/css/wp-verifier-tabs.css', WP_PLUGIN_CHECK_MAIN_FILE ),
			array(),
			WP_PLUGIN_CHECK_VERSION
		);
	...
	public function render_page() {
		...
		// Load tabs class
		if ( ! class_exists( 'WordPress\\Plugin_Check\\Admin\\Settings_Page_Tabs' ) ) {
			require_once WP_PLUGIN_CHECK_PLUGIN_DIR_PATH . 'includes/Admin/Settings_Page_Tabs.php';
		}
		...
		<div class="wrap">
			...
			<?php Settings_Page_Tabs::render_tabs(); ?>
			...
			switch ( $current_tab ) {
				...
				case 'checks':
				case 'advanced':
					Settings_Page_Tabs::render_coming_soon();
					break;
				default:
					$this->render_ai_tab();
					break;
	...
	/**
	 * Renders the AI Integration tab content.
	 *
	 * @since 1.8.0
	 */
	...
	protected function get_available_providers() {
		$providers_config = require WP_PLUGIN_CHECK_PLUGIN_DIR_PATH . 'includes/AI_Providers.php';
		$providers = array();
		foreach ( $providers_config as $key => $config ) {
			$providers[ $key ] = $config['label'];
		}
		return $providers;
	}
	...
	protected function get_models_for_provider( $provider, $api_key = '' ) {
		$providers_config = require WP_PLUGIN_CHECK_PLUGIN_DIR_PATH . 'includes/AI_Providers.php';
		return $providers_config[ $provider ]['models'] ?? array();
	}
}
